...
// src/analyzer.js
async function initParser({ runtimeWasm, langWasm }) {
  await Parser.init(
    runtimeWasm ? { locateFile: () => runtimeWasm } : void 0
  );
...
function analyze(language, parser, sourceCode) {
...
// src/sample-code.js
var SAMPLE_TS = `import { fetchUser } from './api/user';
...
// src/run-spike.js
async function runSpike(contextName) {
  const report = {
    context: contextName,
    ok: false,
...
async function runOffscreen() {
  try {
    await chrome.offscreen.createDocument({
      url: "offscreen.html",
      reasons: ["DOM_PARSER"],
      justification: "phase0 spike: verify tree-sitter WASM in a DOM-having context"
    });
...
async function main() {
  ...
  await chrome.storage.local.set({
    phase0: { sw, offscreen, finishedAt: (/* @__PURE__ */ new Date()).toISOString() }
  });
